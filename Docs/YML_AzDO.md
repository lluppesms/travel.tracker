...
The Bicep parameter file (`infra/Bicep/main.bicepparam`) uses `#{TOKEN}#` placeholders that are replaced at deploy time by the token-replacement task using Azure DevOps **Variables** and **Variable Groups**. The table below lists every token, whether it should be stored as a plain variable or a secret, and its purpose.

### Azure DevOps Variables (plain)
